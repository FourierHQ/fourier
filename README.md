...
- **A page group opens like a page.** Its drawer is the group's row on either basis, with every section the page drawer has, plus the pages inside it (each opening its own drawer), where visitors went when they *left* the group rather than which of its pages they read next, went on to convert one goal at a time, and how many of the period's conversions happened on its pages or right after one.
