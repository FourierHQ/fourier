...
- **Conversions are credited two ways, side by side**: to the visit they happened in, and to whatever first brought that person to the site. Both count the same conversions and total the same number, so the gap between them is the finding — a channel that introduces people who convert later through something else is doing work no session-scoped report can show. "Introduced them" means the first arrival Fourier recorded, so on a young install a large Direct share is the age of the data rather than a channel.
