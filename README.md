...
- **"Which page drives conversions" excludes the page the conversion happened on.** If the goal fires on `/book-a-demo` then that page tops any naive ranking by construction, and the report has told you where your form is. Conversions credits the last page *before* it — the one that sent them there — and says so.
- **The same conversions are credited three ways**: the visit they happened in, the person's first ever arrival, and their last campaign or referral before it. All three total the same number, which is why they sit in one table. A channel that is small under the first and large under the second is doing work session-scoped reporting cannot see.
