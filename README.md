...
Some of what arrives is instrumentation rather than activity. **Settings → Events** lists the event names that are kept out of the reports, and `$page_leave` — the beacon the SDK sends to report measured attention — is there by default: it fires once per page view, so left in it would roughly double the event count of a site that only has page views, to say something the engagement numbers already say.

A hidden event leaves **every** count, chart, ranking and feed: the overview totals and activity chart, the events list and its filter, top events, and each person's and each company's event count. Nothing is deleted and nothing is dropped at ingest — the rows are still stored, still counted by raw SQL, and showing an event again brings back its entire history, including the part recorded while it was hidden. Like goals and page groups, the setting is stored once for the whole install rather than per environment.

Raw SQL is the one exception, because it is your SQL: `describe_schema` names the hidden events so an agent can exclude them and get the same numbers the dashboard shows.
