...
const endpoints: [string, string, string][] = [
  ...
  ["GET", "/api/projects/default/events?event=&source=&distinct_id=&group_id=&q=&properties=&before=&limit=", "Raw events, newest first, cursor via before=. properties= is a JSON array of filters: [{\"key\":\"plan\",\"op\":\"eq\",\"value\":\"pro\"}]. Most list endpoints accept source="],
  ["GET", "/api/projects/default/events/totals?event=&q=&properties=", "Events, sessions and people matching the same filters as /events"],
  ["GET", "/api/projects/default/timeseries?event=&group_id=&properties=&interval=hour|day|week|month", "Counts and unique users per bucket"],
  ["GET", "/api/projects/default/properties?event=&key=", "Property keys an event carries (every event's without event=), or with key= the values one takes"],
