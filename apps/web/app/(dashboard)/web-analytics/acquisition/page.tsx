...
const GROUPS = [
  ...
  { value: "source", label: "Sources", column: "Source", param: P.utmSource },
...
export default function AcquisitionPage() {
  ...
  const drill = (row: BreakdownRow) => {
    ...
    if (grouping === "channel") next.group_by = "source";
    else if (grouping === "source") next.group_by = "campaign";
    set(next);
  };
            ...
            <CardContent className="p-0">
              <Panel error={errorOf(report.data?.performance)} onRetry={() => report.refetch()}>
                <RankedTable<BreakdownRow>
                  ...
                  sort={sort}
                  onSort={(k) => set({ sort: k })}
                  onSelect={grouping === "campaign" ? undefined : drill}
                  ...
                  caption={
                    <>
                      ...
                      noise, not a winner.
                      {grouping !== "campaign" && " Select a row to drill in."}
                    </>
                  }
                  columns={[
                    {
