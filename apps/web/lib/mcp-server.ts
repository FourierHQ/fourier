...
export function registerFourierTools(server: McpServer) {
  ...
  server.registerTool(
    "list_event_names",
    {
      ...
      inputSchema: z.object({ project_id: projectArg, days: z.number().int().positive().optional().describe("Only the last N days") }),
      annotations: readOnly,
    },
    async ({ project_id, days }) => text(await listEventNames((await project(project_id)).id, { days })),
  );
  ...
  server.registerTool(
    ...
    async ({ project_id, distinct_id, group_id, q, ...rest }) =>
      text(await listEvents((await project(project_id)).id, { ...rest, distinctId: distinct_id, groupId: group_id, search: q })),
  );
  ...
  server.registerTool(
    ...
    async ({ project_id, group_id, ...rest }) => text(await eventTimeseries((await project(project_id)).id, { ...rest, groupId: group_id })),
  );
  ...
  server.registerTool(
    ...
    async ({ project_id, q, identified_only, group_id, order_by, ...rest }) =>
      text(await listUsers((await project(project_id)).id, { ...rest, search: q, identifiedOnly: identified_only, groupId: group_id, orderBy: order_by })),
  );
  ...
  server.registerTool(
    ...
    async ({ project_id, person_id, group_id, exclude_direct, ...rest }) =>
      text(await listTouches((await project(project_id)).id, { ...rest, personId: person_id, groupId: group_id, excludeDirect: exclude_direct })),
  );
  ...
  server.registerTool(
    "attribution_report",
    {
      ...
      inputSchema: z.object({
        ...
        by: z.enum(["utm_source", "utm_medium", "utm_campaign", "referrer_host", "landing_path", "kind"]).optional(),
        identified_only: z.boolean().optional(),
    ...
    async ({ project_id, identified_only, group_id, ...rest }) =>
      text(await attributionReport((await project(project_id)).id, { ...rest, identifiedOnly: identified_only, groupId: group_id })),
  );
